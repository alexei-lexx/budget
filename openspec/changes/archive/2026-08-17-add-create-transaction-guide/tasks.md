...
- [x] 2.1 (use `testing` skill) Extend `load-guides.test.ts`: requesting `["create-transaction"]` returns one guide object with `name`, `token`, and `instruction`; requesting `["basics", "create-transaction"]` returns both, each token paired to its own guide
- [x] 2.2 Run the extended tests against the existing `load-guides.ts` implementation and confirm it passes unchanged — it already iterates generically over `GUIDES`, so no production code change is expected here

## 3. Gate the Create Transaction Tool

- [x] 3.1 (use `testing` skill) Add rejection-path tests to `create-transaction.test.ts`: missing/invalid `create-transaction` token with a valid `basics` token is rejected, missing/invalid `basics` token with a valid `create-transaction` token is rejected, both missing is rejected, each failure names the missing guide(s), and no transaction is created in any case
- [x] 3.2 (use `testing` skill) Update the existing success-path tests in `create-transaction.test.ts` to pass valid `basics` and `create-transaction` tokens
- [x] 3.3 Add `create-transaction` to `requiredGuides` in `create-transaction.ts` (`["basics", "create-transaction"]`)

## 4. Validation

- [x] 4.1 Run the tests for every changed file, then the full backend suite (`npm test` in `backend/`)
- [x] 4.2 Run `npm run typecheck` and `npm run format` in `backend/` and resolve all issues
- [x] 4.3 Connect an MCP client to the running backend and confirm: `create_transaction` called with only a valid `basics` token is rejected, `load_guides` with `["basics", "create-transaction"]` returns both guides and tokens, and `create_transaction` with both tokens succeeds
