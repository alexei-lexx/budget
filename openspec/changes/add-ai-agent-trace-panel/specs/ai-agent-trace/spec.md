...
The system SHALL display a trigger button near the send button on each AI-powered page. The trigger button SHALL be hidden before any request has been made and while a request is in progress. It SHALL become visible once a response has been received and remain visible until the next request begins.

#### Scenario: Trigger button is hidden before the first request

- **WHEN** the user opens an AI-powered page and has not yet submitted a request
- **THEN** the trigger button is not visible

#### Scenario: Trigger button appears after a response is received

- **WHEN** the AI response completes successfully
- **THEN** the trigger button becomes visible near the send button

#### Scenario: Trigger button is hidden while a request is in progress

- **WHEN** the user submits a new request and the AI is processing
- **THEN** the trigger button is not visible

#### Scenario: Trigger button reappears after subsequent responses

- **WHEN** the user submits a second request and the AI responds
- **THEN** the trigger button is visible and reflects the latest response trace
