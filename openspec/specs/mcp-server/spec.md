...
This domain covers exposing the user's financial data and actions to AI agents via the Model Context Protocol (MCP): token-based authentication of MCP requests, and the MCP tools for listing accounts, categories, and transactions, and creating and updating accounts, categories, and transactions.
